...
while ($row_cursos = mysqli_fetch_array($result_cursos)) {
  ...
  $asignaturas = array();
  $result_asignaturas = mysqli_query($db_con, "SELECT DISTINCT `nombre` FROM `asignaturas` WHERE `curso` = '" . $row_cursos['nomcurso'] . "' AND `nombre` NOT LIKE '%**%' AND `abrev` NOT LIKE '%\_%' AND `nombre` NOT LIKE '%Tutoría%' AND `nombre` <> 'Pedagogía Terapéutica' AND nombre <> 'Refuerzo Pedagógico' ORDER BY `nombre` ASC");
