		...
		<?php
		$coordenadas = $this->config->item('centro_coordenadas');

		$exp_coordenadas = explode(', ', $this->config->item('centro_coordenadas'));
		$coordenadas_latitud = $exp_coordenadas[0];
		$coordenadas_longitud = $exp_coordenadas[1];
		?>
		<!-- GOOGLE MAPS -->
		<script src="https://maps.googleapis.com/maps/api/js?signed_in=true&amp;v=3.exp"></script>
		<script>
        function initialize() {
          var myLatlng = new google.maps.LatLng(<?php echo $coordenadas; ?>);
          var mapOptions = {
            zoom: 15,
            scrollwheel: false,
            center: {lat: <?php echo $coordenadas_latitud; ?>, lng: <?php echo $coordenadas_longitud; ?>},
